...
var SIOProxyConnection = function (id, conn, server) {
  ...
  conn.once("disconnect", function () {
    self.disconnect("engine.io client disconnected.");
  });
...
SIOProxyConnection.prototype.is_ssl = function () {
  var self = this;

  // Trust localhost as if it were secure. Apache only allows websockets over SSL, so this is OK.
  if (self.conn.handshake.address.address === "127.0.0.1" && self.conn.handshake.headers["x-forwarded-for"]) {
...
SIOProxyConnection.prototype.pipe_data = function (err) {
  var self = this;
...
SIOProxyConnection.prototype._remote_address = function () {
  ...
  try {
    if (self.conn.handshake.address.address === "127.0.0.1" && self.conn.handshake.headers["x-forwarded-for"]) {
      return self.conn.handshake.headers["x-forwarded-for"];
    }
...
SIOProxyConnection.prototype.disconnect = function (err) {
  ...
  if (self.conn) {
    self.conn = null;
    try {
      if (conn.manager.transports[conn.id] && conn.manager.transports[conn.id].open) {
        log.log("forcing disconnect for conn %s", conn.id);
        conn.manager.transports[conn.id].end();
        // Using conn.manager.transports[conn.id].onForcedDisconnect(); stops the client from reconnecting
      } else {
        //debugger;
        log.log("using old-style disconnect on conn %s", conn.id);
        conn.disconnect();
      }
      // don't work
      // conn.end();
      // conn.onclose();
      // doesn't seem to send anything to the client. just times out
      //conn.manager.onClientDisconnect(conn.id);
