...
IRCMultiplexer.prototype.handle_colab_msg = function (p, msg) {
  ...
  case "join":
    ...
    usernames = _.uniq(_.pluck(colab_conn.users, "username"));
    if (_.contains(usernames, msg.username)) {
      break;
    }
    if (self.colab_conn) {
      self.colab_conn.users[msg.user_id] = msg;
    } else {
      log.warn("No colab connection for %s. Can't set user %s", self.toString(), JSON.stringify(msg));
    }
  ...
  case "part":
    if (self.colab_conn) {
      ...
      log.warn("No colab connection for %s. Can't delete user %s", self.toString(), JSON.stringify(msg));
    }
    usernames = _.uniq(_.pluck(colab_conn.users, "username"));
    if (_.contains(usernames, msg.username)) {
      break;
