...
IRCProxyConnection.prototype._on_mode = function (msg) {
  ...
  if (target === self.username) {
    if (msg.args[1] !== "+i") {
      self.write("%s :Unknown mode %s", IRC_ERR.UMODEUNKNOWNFLAG, msg.args[1]);
      return;
    }

    self.write("%s %s", IRC_RES.UMODEIS, msg.args[1]);
  } else if (target === "#" + self.path) {
    // TODO
  } else {
    self.write("%s :Username doesn't match %s", IRC_ERR.USERSDONTMATCH, msg.args[0]);
    return;
