...
IRCProxyConnection.prototype.check_auth = function () {
  ...
  // 002: Your host is sendak.freenode.net[2001:778:627f::1:0:49/6667], running version ircd-seven-1.1.3
  // 003: This server was created Tue Apr 8 2014 at 17:59:15 EEST
  // 004: sendak.freenode.net ircd-seven-1.1.3 DOQRSZaghilopswz CFILMPQSbcefgijklmnopqrstvz bkloveqjfI
  // 005: CHANTYPES=#~ EXCEPTS INVEX CHANMODES=eIbq,k,flj,CFLMPQScgimnprstz CHANLIMIT=#:120 PREFIX=(ov)@+ MAXLIST=bqeI:100 MODES=4 NETWORK=freenode KNOCK STATUSMSG=@+ CALLERID=g are supported by this server
  // 005: CASEMAPPING=rfc1459 CHARSET=ascii NICKLEN=16 CHANNELLEN=50 TOPICLEN=390 ETRACE CPRIVMSG CNOTICE DEAF=D MONITOR=100 FNC TARGMAX=NAMES:1,LIST:1,KICK:1,WHOIS:1,PRIVMSG:4,NOTICE:4,ACCEPT:,MONITOR: are supported by this server
  // 005: EXTBAN=$,ajrxz WHOX CLIENTVER=3.0 SAFELIST ELIST=CTU are supported by this server

  if (self.username && self.api_secret && !self.sent_welcome) {
    self.sent_welcome = true;
    self.write("%s %s :Welcome to Floobits IRC", IRC_RES.WELCOME, self.username);
    self.write("%s %s", IRC_RES.YOURHOST, settings.irc_server_name);
    self.write("%s %s", IRC_RES.CREATED, "Jan 1 1970");
    self.write("%s %s", IRC_RES.MYINFO, "Floobits IRC");
    self.write("%s %s", IRC_RES.BOUNCE, "CHANTYPES=# NETWORK=Floobits");
  }
...
IRCProxyConnection.prototype._on_who = function (msg) {
  var self = this,
    target = msg.args[0];
  ...
  _.each(self.users, function (user) {
    self.write("%s %s %s %s %s %s %s H :0 %s", IRC_RES.WHOREPLY, self.username, target,  user.username, "?", "?", user.username, user.username);
  });

  self.write("%s %s %s :End of /WHO list", IRC_RES.ENDOFWHO, self.username, target);
};

IRCProxyConnection.prototype._on_mode = function (msg) {
  var self = this,
    target = msg.args[0],
    mode = msg.args[1];
  ...
  if (target === self.username) {
    ...
    self.write("%s %s", IRC_RES.UMODEIS, mode);
  } else if (target === "#" + self.path) {
    if (mode === "b") {
      self.write("%s #%s End of Channel Ban List", IRC_RES.ENDOFBANLIST, self.path);
    }
  } else {
...
IRCProxyConnection.prototype._on_names = function (msg) {
  var self = this,
    targets = msg.args[0],
    usernames = _.uniq(_.pluck(self.users, "username")).join(" ");

  if (targets) {
    targets = targets.split(",");
  }
  self.write("%s %s #%s :No topic is set", IRC_RES.NOTOPIC, self.username, self.path);
  if (usernames.length > 0) {
    ...
    log.warn("No usernames for %s. self.users is %s", self.path, JSON.stringify(self.users));
  }
  self.write("%s %s #%s :End of /NAMES list", IRC_RES.ENDOFNAMES, self.username, self.path);
};
...
IRCProxyConnection.prototype.handle_colab_msg = function (msg) {
  ...
  case "room_info":
    ...
    self.write("JOIN #%s", self.path, self.username);
    self._on_names({args: []});
    break;
  case "join":
    self.write("JOIN #%s", self.path, msg.username);
    break;
  case "part":
    self.write("PART #%s", self.path, msg.username);
    break;
  default:
