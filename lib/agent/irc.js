...
IRCMultiplexer.prototype.reply = function () {
  ...
  args = args.slice(1);
  msg = util.format(":%s %s %s %s", settings.irc_server_name, code, self.username, util.format.apply(null, args));
  self.write(msg);
};
...
IRCMultiplexer.prototype.welcome = function () {
  ...
  self.reply(IRC_RES.WELCOME, ":Welcome to Floobits IRC, %s!%s@%s", self.username, self.username, self.remote_address);
