...
IRCMultiplexer.prototype.welcome = function () {
  ...
  self.reply(IRC_RES.WELCOME, "%s :Welcome to Floobits IRC, %s!%s@%s", self.username, self.username, self.username, self.remote_address);
  ...
  self.reply(IRC_RES.BOUNCE, ":CHANTYPES=# EXCEPTS INVEX CHANMODES=eIbq,k,flj,CFLMPQScgimnprstz CHANLIMIT=#:120 PREFIX=(ov)@+ MAXLIST=bqeI:100 MODES=4 NETWORK=Floobits CALLERID=g are supported by this server");
  self.reply(IRC_RES.BOUNCE, ":EXTBAN=$,ajrxz WHOX CLIENTVER=3.0 SAFELIST ELIST=CTU are supported by this server");
  ...
  self.write(":%s!%s@%s %s MODE +i", self.username, self.username, self.remote_address, self.username);
};
...
IRCMultiplexer.prototype.forward_data = function (target, err, reason) {
  ...
  self.write("JOIN #%s", target);
  self.write("MODE #%s +ns", target);
