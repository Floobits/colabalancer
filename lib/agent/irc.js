...
IRCProxyConnection.prototype.check_auth = function (p) {
  var self = this,
    colab_conn = p && self.colab_conns[p];
  ...
  if (!self.username || !self.api_secret || !colab_conn) {
...
IRCProxyConnection.prototype._on_privmsg = function (msg) {
  ...
  colab_conn.write(JSON.stringify({
    name: "msg",
    data: txt.slice(1)
  }) + "\n");
};
...
IRCProxyConnection.prototype._on_names = function (msg) {
  ...
  _.each(targets, function (target) {
    var colab_conn = self.colab_conns[target],
      usernames;
    ...
    self.reply(IRC_RES.ENDOFNAMES, "#%s :End of /NAMES list.", self.username, target);
  });
};
...
IRCProxyConnection.prototype.forward_data = function (p, err, reason) {
  var self = this,
    colab_conn;
  ...
  colab_conn.on("data", self.on_colab_data.bind(self, p));
  colab_conn.on("end", self.colab_disconnect.bind(self, p));
  colab_conn.on("error", self.colab_disconnect.bind(self, p));
};

IRCProxyConnection.prototype.handle_colab_msg = function (p, msg) {
  var self = this,
    colab_conn = self.colab_conns[p];
...
IRCProxyConnection.prototype.colab_disconnect = function (p, err, reason) {
  var self = this,
    colab_conn = self.colab_conns[p];


  if (!colab_conn) {
    log.error("No colab conn for path %s", p);
    return;
  }

  // TODO: only disconnect colab_conn and leave the associated channel
  self.disconnect(err, reason);
  delete self.colab_conns[p];
};
