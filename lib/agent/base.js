...
var BaseProxyConnection = function (id, conn, server) {
  ...
  self.is_ssl = self._is_ssl(conn, server);
...
BaseProxyConnection.prototype._is_ssl = function (conn, server) {
  var ssl_servers = [server.server_ssl, server.io_ssl.server, server.irc_server_ssl];

  if (conn) {
    // TODO: one of these first two can be removed, since it was for socket.io
    if (conn.manager && _.contains(ssl_servers, conn.manager.server)) {
      return true;
    }
    if (conn.socket && _.contains(ssl_servers, conn.socket.server)) {
      return true;
    }
    // Websocket
    if (conn.server && _.contains(ssl_servers, conn.server)) {
      return true;
    }
  }
  return false;
