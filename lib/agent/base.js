...
BaseProxyConnection.prototype.is_ssl = function () {
  var self = this,
    ssl_servers = [self.server.server_ssl, self.server.io_ssl.server, self.server.irc_server_ssl];

  if (self.conn) {
    // TODO: one of these first two can be removed, since it was for socket.io
    if (self.conn.manager && _.contains(ssl_servers, self.conn.manager.server)) {
      return true;
    }
    if (self.conn.socket && _.contains(ssl_servers, self.conn.socket.server)) {
      return true;
    }
    // Websocket
    if (self.conn.server && _.contains(ssl_servers, self.conn.server)) {
      return true;
    }
  }
  return false;
