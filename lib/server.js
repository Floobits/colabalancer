...
var Sio_mc_store = require("socket.io-store-memcached");
...
ColaBalancerServer.prototype.listen = function () {
  ...
  self.io.configure(function () {
    ...
    self.io.set("logger", log);
    self.io.set("log level", 3);
    self.io.set("store", new Sio_mc_store({ hosts: settings.cache_servers }));
  });
  ...
  if (self.https_server) {
    ...
    self.io_ssl.configure(function () {
      ...
      self.io_ssl.set("logger", log);
      self.io_ssl.set("log level", 3);
      self.io_ssl.set("store", new Sio_mc_store({ hosts: settings.cache_servers }));
    });
...
ColaBalancerServer.prototype.on_conn = function (conn) {
  ...
  if (settings.conn_timeout) {
    conn.setTimeout(settings.conn_timeout, function () {
      log.error("%s:%s timed out. Disconnecting.", conn.remoteAddress, conn.remotePort);
      conn.end();
      conn.destroy();
    });
  }
