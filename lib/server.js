...
var ColaBalancerServer = function () {
  ...
  if (settings.json_port_ssl) {
    ...
    self.server_ssl = tls.createServer(tls_options, self.on_conn.bind(self, agent.ProxyConnection));
  }
  ...
  if (settings.irc_port_ssl) {
    ...
    self.irc_server_ssl = tls.createServer(tls_options, self.on_conn.bind(self, agent.IRCMultiplexer));
  }
...
ColaBalancerServer.prototype.on_conn = function (AgentConnection, conn) {
  var self = this,
    agent_conn,
    number;
...
ColaBalancerServer.prototype.on_sio_conn = function (socket) {
  ...
  self.on_conn(agent.EIOProxyConnection, socket);
};
